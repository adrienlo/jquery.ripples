	...
	var Ripples = function (el, options) {
		...
		$window.on('resize', function() {
			if(FIXED) {
				canvas.width = _window.innerWidth;
				canvas.height = _window.innerHeight;
			} else if (that.$el.innerWidth() != that.canvas.width || that.$el.innerHeight() != that.canvas.height) {
	...
	Ripples.prototype = {
		...
		computeTextureBoundaries: function() {
			...
			if(FIXED) {
				...
				winWidth = parElement.innerWidth();
				winHeight = parElement.innerHeight();
			}
			...
			if(FIXED) {
				this.renderProgram.uniforms.bottomRight = new Float32Array([
					this.renderProgram.uniforms.topLeft[0] + _window.innerWidth / backgroundWidth,
					this.renderProgram.uniforms.topLeft[1] + _window.innerHeight / backgroundHeight
				]);
			} else {
				this.renderProgram.uniforms.bottomRight = new Float32Array([
